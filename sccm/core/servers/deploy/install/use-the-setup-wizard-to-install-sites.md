---
title: "Assistente de Instalação | System Center Configuration Manager"
ms.custom: na
ms.date: 10/06/2016
...
## <a name="a-namebkmkprimarya-install-a-central-administration-site-or-primary-site"></a><a name="bkmk_primary"></a> Instalar um site de administração central ou site primário
...
### <a name="a-namebkmkinstallpria-to-install-a-primary-or-central-administration-site"></a><a name="bkmk_installpri"></a> Para instalar um site de administração central ou primário
...
3.  Na página **Introdução**, escolha o tipo de site que deseja instalar:  
     ...
     **Site primário** — como um site primário autônomo, que é o primeiro site de uma nova hierarquia, ou um primário filho:  

    -   Selecione: **Instalar um site primário do Configuration Manager**  

        > [!TIP]  
        >  Normalmente, você seleciona apenas **Usar opções de instalação típicas de um site primário autônomo** quando deseja instalar o site primário autônomo em um ambiente de teste. Quando você seleciona essa opção:  
        >   
        > -   A instalação configura automaticamente o site como um site primário autônomo  
        > -   É usado um caminho de instalação padrão  
        > -   É usada uma instalação local da instância padrão do SQL Server para o banco de dados do site  
        > -   É instalado um ponto de gerenciamento e um ponto de distribuição no computador do servidor do site  
        > -   Será configurado o site com o idioma inglês e o idioma de exibição do sistema operacional no servidor do site primário, se ele corresponder a um dos idiomas a que o Configuration Manager dá suporte  
...
4.  Na página **Chave do Produto (Product Key)**:
    - escolha se deseja instalar o Configuration Manager como uma edição de avaliação ou uma edição licenciada.  
      ...
      -   Se você selecionar uma edição licenciada, insira a chave do produto (Product Key) e clique em **Avançar**.  

      -   Se você selecionar uma edição de avaliação, clique em **Avançar**. (Você pode atualizar uma instalação de avaliação para uma instalação completa mais tarde.)  
...
6.  Na página **Licenças de Pré-requisito** , leia e aceite os termos de licença do software de pré-requisito.  A Instalação baixa e instala automaticamente o software nos clientes ou sistemas do site, quando necessário. Marque todas as caixas de seleção para avançar para a próxima página.  
...
8.  Na página **Seleção do Idioma do Servidor**, selecione os idiomas que estão disponíveis para o console do Configuration Manager e para os relatórios. (O inglês é o idioma selecionado por padrão e não pode ser removido.)  

9. Na página **Seleção do Idioma do Cliente** , selecione os idiomas que estão disponíveis em computadores cliente, especifique se deseja habilitar todos os idiomas do cliente para clientes de dispositivos móveis. (O inglês é o idioma selecionado por padrão e não pode ser removido.)  
...
10. Na página **Configurações do Site e de Instalação** , especifique o seguinte para o novo site que você está instalando:  
    ...
    -   **Código do site**: [cada código do site em uma hierarquia deve ser exclusivo](../../../../core/servers/deploy/install/prepare-to-install-sites.md#bkmk_sitecodes) e deve ser composto por três dígitos alfanuméricos (A até Z e 0 até 9). Como o código do site é usado em nomes de pasta, não use os itens listados a seguir para o site, o incluindo nomes reservados do Windows:    
        -   AUX  
        -   CON    
        -   NUL    
        -   PRN    
    ...
    -   **Pasta de instalação**: é o caminho de pasta para a instalação do Configuration Manager. Você não pode alterar o local após a instalação do site e o caminho não pode conter caracteres Unicode ou espaços à direita.  
...
11. Na página **Instalação do Site** , use a opção a seguir que corresponde ao seu cenário:  
    ...
    -   **Estou instalando um site de administração central:**  
         ...
         Na página **Instalação do Site de Administração Central** , selecione **Instalar como o primeiro site em uma nova hierarquia**e clique em **Avançar** para continuar.  

    -   **Estou expandindo um site primário autônomo em uma hierarquia com um site de administração central:**  

         Na página **Instalação do Site de Administração Central** , selecione **Expandir um primário autônomo existente em uma hierarquia**, especifique o FQDN do servidor do site primário autônomo e clique em **Avançar** para continuar.  
    ...
    -   **Estou instalando um site primário autônomo:**  
         ...
         Na página **Instalação de Site Primário** , selecione**Instalar o site primário como site autônomo**e clique em **Avançar**.  

    -   **Estou instalando um site primário filho:**  

         Na página **Instalação de Site Primário** , selecione **Ingressar o site primário em uma hierarquia existente**, especifique o FQDN para o site de administração central e clique em **Próximo**.  
...
12. Na página Informações do Banco de Dados, especifique as informações a seguir:  
    ...
    -   **Nome da instância:** por padrão, fica em branco, usando a instância padrão do SQL no computador do servidor do site.  
    ...
    -   **Porta do Service Broker:** por padrão, essa opção é definida para usar a porta padrão 4022 do SSB (SQL Server Service Broker), a qual é usada pelo SQL para se comunicar diretamente com o banco de dados do site em outros sites.  

13. Na segunda **Informações do Banco de Dados** , você pode especificar locais não padrão para o arquivo de dados do SQL Server e o arquivo de log do SQL Server para o banco de dados do site:  
...
15. Na página **Configurações de Comunicação do Cliente** , escolha se é para configurar todos os sistemas de sites para aceitar somente a comunicação HTTPS de clientes ou se o método de comunicação deve ser configurado para cada função do sistema de sites.  
    ...
    -   Quando você seleciona **Todas as funções do sistema de sites aceitam apenas comunicação HTTPS de clientes**, o computador cliente deve ter um certificado PKI válido para autenticação do cliente. Para obter mais informações sobre os requisitos de certificado PKI, consulte Requisitos de certificado PKI para o Configuration Manager  
...
16. Na página **Funções do Sistema de Site** , escolha se é para instalar um ponto de gerenciamento ou um ponto de distribuição. Para cada função que você opta que o programa de instalação instale:  
    ...
    -   Se você selecionou **Todas as funções do sistema de site aceitam apenas comunicação HTTPS de clientes** na página anterior, as definições de conexão do cliente serão automaticamente configuradas para HTTPS e não poderão ser alteradas a menos que você volte e mude a configuração.  
...
17. Na página **Dados de Uso** , examine as informações sobre os dados que a Microsoft coleta e clique em **Avançar**.  

18. A **Configuração de Ponto de Conexão de Serviço** só está disponível durante a instalação quando:  

    -   Você estiver instalando um site primário autônomo  

    -   Você estiver instalando um site de administração central  
    ...
    >  Se você estiver instalando um site primário filho, ignore esta etapa (esta página não estará disponível).  
     ...
     Depois de selecionar uma configuração para o **Ponto de Conexão de Serviço**, clique em **Avançar**. (Após a conclusão da Instalação, você poderá alterar essa configuração no console do Configuration Manager.)  

19. Na página **Resumo das Configurações** , examine a configuração selecionada e, quando estiver pronto, clique em **Avançar** para iniciar o Verificador de Pré-requisitos.  

20. Na página **Verificação de Instalação de Pré-requisitos** , todos os problemas que podem ser identificados serão listados.  

    -   Quando o Verificador de Pré-requisitos encontrar um problema, clique em um item da lista para exibir os detalhes sobre como solucionar o problema.  

    -   Você deve resolver todo item com status de **Falha** antes de continuar a instalar o site. Itens com o status de **Aviso** devem ser resolvidos, mas não bloquearão a instalação do site.  

    -   Depois de resolver os problemas, clique em **Executar Verificação** para executar novamente o verificador de pré-requisitos.  

     Se o verificador de pré-requisitos for executado e nenhuma verificação receber um status de **Falha** , você poderá clicar em **Iniciar Instalação** para iniciar a instalação do site.  

    > [!TIP]  
    >  Além dos comentários fornecidos no Assistente, você pode encontrar informações adicionais sobre problemas de pré-requisito ao exibir o arquivo **ConfigMgrPrereq.log** na raiz da unidade do sistema do computador que você está instalando.  Para obter uma lista das regras e descrições dos pré-requisitos de instalação, consulte [Lista de verificações de pré-requisitos para o System Center Configuration Manager](../../../../core/servers/deploy/install/list-of-prerequisite-checks.md).  

21. Na página **Instalação** , o programa de instalação exibe o status da instalação. Quando a instalação do servidor do site principal for concluída, você verá a opção **Fechar** no Assistente de instalação. Quando você fecha o assistente, a instalação e as configurações iniciais do site continuam em segundo plano.  

    -   Você pode conectar um console do Configuration Manager ao site antes da instalação ser concluída. Esse console se conecta no modo somente leitura e permite a exibição de objetos e de configurações, mas não a introdução de edições.  

    -   Após a conclusão da instalação, você poderá conectar um console que permite a edição de objetos e de configurações.  


## <a name="a-namebkmkexpanda-expand-a-stand-alone-primary-site"></a><a name="bkmk_expand"></a> Expandir um site primário autônomo
Ao instalar um site primário autônomo como seu primeiro site, você terá a opção de expandir mais tarde o site em uma hierarquia maior ao instalar um site de administração central.   
...
Para expandir um site primário autônomo, use o procedimento *[Para instalar um site de administração central ou primário](../../../../core/servers/deploy/install/use-the-setup-wizard-to-install-sites.md#bkmk_installpri)*, encontrado neste artigo.


## <a name="a-namebkmksecondarya-install-a-secondary-site"></a><a name="bkmk_secondary"></a> Instalar um site secundário
...
### <a name="a-namebkmkinstallsecondarya-to-install-a-secondary-site"></a><a name="bkmk_installsecondary"></a> Para instalar um site secundário  


1.  No console do Configuration Manager, navegue até **Administração** > **Configuração de Site** > **Sites**e selecione o site que será o site primário pai do novo site secundário  

2.  Clique em **Criar Site Secundário** para iniciar o **Assistente de Criação de Site Secundário**.  

3.  Na página **Antes de Começar** , confirme se o site primário listado é o site que você deseja que seja o pai do novo site secundário e clique em **Avançar**.  
...
4.  Na página **Geral** , especifique o seguinte:  
    ...
    -   **Pasta de instalação**: esse é o caminho de pasta para a instalação do Configuration Manager. Você não pode alterar o local após a instalação do site e o caminho não pode conter caracteres Unicode ou espaços à direita.  

    > [!IMPORTANT]  
    >  Depois de especificar detalhes nesta página, você pode clicar em **Resumo** para usar as configurações padrão para o restante das opções de site secundário e ir diretamente para a página **Resumo** do assistente.  
    >   
    >  -   Só use essa opção quando estiver familiarizado com as configurações padrão do assistente e elas forem as configurações que você deseja usar.  
...
5.  Na página **Arquivos de Origem de Instalação** , escolha como o computador do site secundário obtém os arquivos de origem para instalar o site.  
     ...
     Quando você usa arquivos de origem armazenados na rede ou armazenados no computador do site secundário:  

    -   O local do arquivo de origem deve incluir uma pasta chamada **Redist** , a qual inclui todos os arquivos baixados anteriormente usando o Downloader de Instalação.  

    -   Se qualquer um dos arquivos de **Redist** não estiver indisponível, haverá falha na instalação do site secundário.  
...
6.  Na página **Configurações do SQL Server** , especifique a versão do SQL Server a ser usada e defina as configurações relacionadas.  
    ...
    -   **FQDN do SQL Server**: examine o FQDN para o computador do SQL Server. É necessário usar um SQL Server local para hospedar o banco de dados do site secundário e não é possível modificar essa configuração.  
    ...
    > [!TIP]  
    >  Consulte [Suporte para versões do SQL Server](../../../../core/plan-design/configs/support-for-sql-server-versions.md) para obter uma lista das versões do SQL Server com suporte com o System Center Configuration Manager.  
...
7.  Na página **Ponto de Distribuição** , defina as configurações para o ponto de distribuição que será instalado no servidor do site secundário.  
    ...
    **Configurações opcionais:**  
    ...
    -   **Instalar e configurar o IIS se exigido pelo Configuration Manager** – selecione essa configuração para permitir que o Configuration Manager instale e configure o ISS (Serviços de Informações da Internet) no servidor se ele ainda não estiver instalado. O IIS deve ser instalado em todos os pontos de distribuição.  
    ...
    -   **Descrição** – descrição amigável do ponto de distribuição para ajudá-lo a reconhecê-lo.  
...
8.  Na página **Configurações de Unidade** , especifique as configurações da unidade para o ponto de distribuição do site secundário.  
    ...
    -   **Locais de Conteúdo**: Especifique os locais de conteúdo para a biblioteca de conteúdo e o compartilhamento de pacotes. O Configuration Manager copia conteúdo para o local de conteúdo primário até que a quantidade de espaço livre atinja o valor especificado para **Reserva de espaço de unidade (MB)**. Por padrão, os locais de conteúdo são definidos como **Automático**; o local de conteúdo primário será atribuído à unidade de disco com mais espaço em disco na instalação, e o local secundário será atribuído à unidade de disco com o segundo maior espaço em disco livre. Quando as unidades primárias e secundárias atingirem a reserva de espaço de unidade, o Configuration Manager selecionará outra unidade disponível com o maior espaço em disco e continuará o processo de cópia.  
...
9. Na página **Validação de Conteúdo** , especifique se a integridade dos arquivos de conteúdo no ponto de distribuição deve ser validada.  
    ...
    -   Para ver os resultados do processo de validação de conteúdo, no console do Configuration Manager, navegue até **Monitoramento** >  **Status da Distribuição** >  **Status do Conteúdo**. O conteúdo de cada tipo de pacote (por exemplo, aplicativo, pacote de atualização de software e imagem de inicialização) é exibido.  

10. Na página **Grupos de Limite** , gerencie os grupos de limite aos quais o ponto de distribuição está atribuído:  
...
11. Na página **Resumo** , verifique as configurações e clique em **Próximo** , para instalar o site secundário. Quando o assistente apresenta a página **Conclusão** , você pode fechar o assistente. A instalação do site secundário continua em segundo plano.  


### <a name="a-namebkmkverifya-to-verify-the-secondary-site-installation-status"></a><a name="bkmk_verify"></a> Para verificar o status da instalação do site secundário  

1.  No console do Configuration Manager, navegue até **Administração** > **Configuração de Site** > **Sites**.  

2.  Selecione o servidor do site secundário que você está instalando e clique em **Mostrar Status da Instalação**.  

    > [!TIP]  
    >  Quando você instala mais de um site secundário de cada vez, a verificação de pré-requisito é executada em um único site por vez e deve ser concluída antes da verificação do site seguinte.  
