...
10. Na página **Configurações do Site e de Instalação**, especifique o seguinte para o novo site que você está instalando:  
    ...
    -   **Código do site:** [cada código do site em uma hierarquia precisa ser exclusivo](../../../../core/servers/deploy/install/prepare-to-install-sites.md#bkmk_sitecodes) e composto por três dígitos alfanuméricos (A a Z e 0 a 9). Como o código do site é usado em nomes de pasta, não use nomes reservados do Windows para o site, incluindo:    
        -   AUX  
        -   CON    
        -   NUL    
        -   PRN    
    ...
    -   **Pasta de instalação:** esse é o caminho da pasta de instalação do Configuration Manager. Você não pode alterar o local depois de instalar o site. Além disso, o caminho não pode conter espaços à direita ou caracteres Unicode.  
...
12. Na página **Informações do Banco de Dados**, especifique as informações a seguir:  
    ...
    - **Nome do SQL Server (FQDN):** por padrão, essa opção é definida como o computador do servidor do site.
    ...
    - **Nome da instância:** por padrão, essa opção fica em branco. Ele usa a instância padrão do SQL no computador do servidor do site.  

    - **Nome do banco de dados:** Por padrão, essa opção é definida como CM_&lt;Sitecode\>. Você é livre para usar um nome diferente que você especificar.  

    - **Porta do Service Broker:** por padrão, essa opção é definida para usar a porta do SSB (SQL Server Service Broker) padrão 4022. O SQL a usa para se comunicar diretamente com o banco de dados do site em outros sites.  
...
4. Na página **Geral** , especifique o seguinte:  
   ...
   - **Código do site**: cada código do site em uma hierarquia precisa ser exclusivo e composto por três dígitos alfanuméricos (A a Z e 0 a 9). Como o código do site é usado em nomes de pasta, não use nomes reservados do Windows para o site, incluindo:  
   ...
   - **Nome do servidor do site**: esse é o FQDN do servidor em que o novo site secundário será instalado.  

   - **Nome do site**: cada site exige esse nome amigável, que pode ajudá-lo a identificar o site.  

   - **Pasta de instalação**: esse é o caminho da pasta de instalação do Configuration Manager. Você não pode alterar o local depois de instalar o site. O caminho não pode conter espaços à direita ou caracteres Unicode.  
...
6. Na página **Configurações do SQL Server** , especifique a versão do SQL Server a ser usada e defina as configurações relacionadas.  
   ...
   -   **Porta do SQL Server Broker**: especifique a porta do SQL Server Service Broker (SSB) a ser usada pelo SQL Server Express. O Service Broker geralmente é configurado para usar a porta TCP 4022, mas você pode configurar outra porta. É necessário especificar uma porta válida que nenhum outro site ou serviço esteja usando e que nenhuma restrição de firewall esteja bloqueando.  
...
7. Na página **Ponto de Distribuição** , defina as configurações para o ponto de distribuição que será instalado no servidor do site secundário.  
   ...
   -   **Criar um certificado autoassinado ou importar um certificado de cliente PKI**: escolha entre usar um certificado autoassinado (que também possibilita permitir conexões anônimas de clientes do Configuration Manager à biblioteca de conteúdo) ou importar um certificado de sua PKI.  
