...
- No **modo online**, o ponto de conexão de serviço se conecta automaticamente ao serviço de nuvem da Microsoft e baixa as atualizações aplicáveis.  
    ...
    Por padrão, o Configuration Manager verifica se há novas atualizações a cada 24 horas. Verifique manualmente se há atualizações no console do Configuration Manager. Vá para o workspace **Administração**, selecione o nó **Atualizações e Manutenção** e escolha **Verificar se há Atualizações** na faixa de opções.  
...
- As atualizações que você instalou são exibidas como **Instaladas**. Apenas a atualização instalada mais recentemente é mostrada. Para exibir as atualizações instaladas anteriormente, selecione **Histórico** na faixa de opções.  
...
- [Lista de verificação para instalar a atualização 1902](/sccm/core/servers/manage/checklist-for-installing-update-1902)
...
3. Selecione **Executar verificação de pré-requisitos** na faixa de opções.  
...
No site de nível superior da sua hierarquia, no console do Configuration Manager, vá para o workspace **Administração** e selecione o nó **Atualizações e manutenção**. Selecione uma atualização com o estado de **Disponível** e, em seguida, escolha **Instalar Pacote de Atualização** na faixa de opções.  
...
1. No console do Configuration Manager, acesse o workspace **Administração**, expanda **Configuração do Site** e selecione o nó **Sites**. Selecione o site secundário que você deseja atualizar e, em seguida, escolha **Atualizar** na faixa de opções.  

2. Selecione **Sim** para iniciar a atualização do site secundário.  

Para monitorar a instalação da atualização em um site secundário, selecione o site secundário e escolha **Mostrar Status da Instalação** na faixa de opções. Adicione também a coluna **Versão** ao nó Sites de modo que possa exibir a versão de cada site secundário.  
...
Vá para o workspace **Monitoramento** e selecione o nó **Status de Manutenção do Site**. Selecione a atualização e, em seguida, escolha uma das seguintes opções:  
...
- **Ignorar avisos de pré-requisito**: se a instalação da atualização for interrompida por causa de um aviso, você poderá selecionar **Ignorar avisos de pré-requisito**. Essa ação permite que a instalação da atualização continue (após alguns minutos) e usa a opção para ignorar os avisos de pré-requisito.  
...
- [Aprovar solicitações de aplicativo para usuários por dispositivo](/sccm/apps/deploy-use/deploy-applications#specify-deployment-settings) <!--1357015-->  
...
- [Executar a etapa de sequência de tarefas](/sccm/osd/deploy-use/manage-task-sequences-to-automate-tasks#add-child-task-sequences-to-a-task-sequence) <!--1261338-->
...
- [Ponto de serviço de data warehouse](/sccm/core/servers/manage/data-warehouse) <!--1277922-->
