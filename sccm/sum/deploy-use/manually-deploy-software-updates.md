...
1.  No console do Configuration Manager, acesse o espaço de trabalho **Biblioteca de Software**, expanda **Atualizações de Software** e clique em **Todas as Atualizações de Software**. Esse nó exibe todas as atualizações de software sincronizadas.  
...
1.  No console do Configuration Manager, acesse o espaço de trabalho **Biblioteca de Software** e selecione **Atualizações de Software**. Selecione as atualizações de software desejadas.  
...
1. Para monitorar o status do conteúdo das atualizações de software, acesse o espaço de trabalho **Monitoramento** no console do Configuration Manager. Expanda **Status de Distribuição** e, em seguida, selecione o nó **Status do Conteúdo**.  
...
1.  No console do Configuration Manager, acesse o espaço de trabalho **Biblioteca de Software**, expanda **Atualizações de Software** e selecione o nó **Grupos de Atualizações de Software**.  
...
3.  Na página **Geral** do Assistente para Implantar Atualizações de Software, defina as seguintes configurações:  
    ...
    -   **Atualização de Software/Grupo de Atualização de Software**: verifique se o grupo de atualização de software ou a atualização de software exibida está correta.  
    ...
    -   **Coleção**: especifique a coleção para a implantação. Os dispositivos na coleção recebem as atualizações de software dessa implantação.  

4.  Na página **Configurações de Implantação**, defina as seguintes configurações:  

    -   **Tipo de implantação**: especifique o tipo de implantação para a implantação de atualização do software.  
...
5.  Na página **Agendamento**, defina as seguintes configurações:  
    ...
    -   **Prazo de instalação**: essas opções só estão disponíveis para implantações **Obrigatórias**. Selecione uma das seguintes configurações para especificar a data limite de instalação das atualizações de software na implantação  
...
6.  Na página **Experiência do Usuário**, defina as seguintes configurações:  
    ...
    -   **Comportamento de reinicialização de dispositivo**: essa configuração só é configurável para implantações **Obrigatórias**. Especifique se deseja suprimir a reinicialização do sistema em servidores e estações de trabalho quando há necessidade de reinicialização para concluir a instalação da atualização.  
...
7. Na página **Alertas**, configure como o Configuration Manager gera alertas para essa implantação. Examine os alertas de atualizações de software recentes do Configuration Manager no nó **Atualizações de Software** do espaço de trabalho **Biblioteca de Software**. Se você também estiver usando o System Center Operations Manager, configure seus alertas. Apenas configure alertas para implantações **Obrigatórias**.  
