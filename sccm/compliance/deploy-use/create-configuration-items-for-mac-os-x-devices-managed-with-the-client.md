...
---
# <a name="create-configuration-items-for-mac-os-x-devices-managed-with-the-system-center-configuration-manager-client"></a>Criar itens de configuração para dispositivos Mac OS X gerenciados com o cliente do System Center Configuration Manager
...
7.  Na página **Plataformas com Suporte**, selecione as versões específicas do Mac OS X que avaliarão o item de configuração.  

8.  Na página **Configurações**, você adicionará novas configurações que serão avaliadas quanto à conformidade em computadores Mac. Clique em **Novo** para abrir a caixa de diálogo **Criar Configuração** .  
...
13. Na página **Regras de Conformidade**, você especificará as condições que definem a conformidade de um item de configuração. Antes que uma configuração possa ser avaliada quanto à conformidade, ela deve ter pelo menos uma regra de conformidade. Clique em **Novo** para adicionar uma nova regra.  
...
14. Na caixa de diálogo **Criar Regra** , forneça as seguintes informações:  
    ...
    -   **Severidade de não conformidade para relatórios** - Especifique o nível de severidade relatado em caso de falha desta regra de conformidade. Os níveis de severidade disponíveis (mostrados em relatórios do Configuration Manager) são:  
        ...
        -   **Crítico com evento** ‑ Computadores que não cumprem essa regra de conformidade relatam uma severidade de falha de **Crítico**. Esse nível de severidade também é registrado pelo computador cliente Mac.  
    ...
    -   Para um tipo de regra de **Existential**, especifique as seguintes informações:  
        ...
        -   **Severidade de não conformidade para relatórios:** Especifique o nível de severidade relatado se a regra de conformidade falhar. Os níveis de severidade disponíveis (mostrados em relatórios do Configuration Manager) são:  
...
Se quiser adicionar esse item de configuração a uma linha de base de configuração, veja [Criar linhas de base de configuração](../../compliance/deploy-use/create-configuration-baselines.md).  
