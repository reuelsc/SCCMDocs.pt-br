...
Implante o sistema operacional em uma coleção de destino. Para obter mais informações, consulte [Deploy a task sequence](manage-task-sequences-to-automate-tasks.md#BKMK_DeployTS). Ao implantar sistemas operacionais usando o PXE, é possível configurar se a implantação será necessária ou estará disponível.
...
-   **Implantação disponível**: as implantações disponíveis exigem que o usuário esteja presente no computador de destino. Um usuário deve pressionar a tecla F12 para continuar o processo de inicialização PXE. Se um usuário não estiver presente para pressionar F12, o computador será inicializado no sistema operacional atual ou no próximo dispositivo de inicialização disponível.
